...
import de.uni.freiburg.iig.telematik.seram.context.data.DUContextProperties;

public class ProcessContextProperties extends DUContextProperties{
	
	private final String CONSTRAINT_FORMAT = ProcessContextProperty.CONSTRAINT + "_%s";
	private final String ACTIVITY_CONSTRAINTS_FORMAT = ProcessContextProperty.ACTIVITY_CONSTRAINTS + "_%s";
	...
	private void addActivityWithConstraints(String activity) {
		...
		setProperty(ProcessContextProperty.ACTIVITIES_WITH_CONSTRAINTS, ArrayUtils.toString(currentActivities.toArray()));
	}
	...
	public Set<String> getActivitiesWithRoutingConstraints(){
		...
		String propertyValue = getProperty(ProcessContextProperty.ACTIVITIES_WITH_CONSTRAINTS);
	...
	private Set<Integer> getConstraintNameIndexes() throws PropertyException{
		...
		for(String constraintName: constraintNames){
			...
			if(separatorIndex == -1 || (constraintName.length() == separatorIndex + 1))
				throw new PropertyException(ProcessContextProperty.CONSTRAINT, constraintName, "Corrupted property file (invalid constraint name)");
			Integer index = null;
			try {
	...
	private void addConstraintNameToList(String constraintName) {
		...
		setProperty(ProcessContextProperty.ALL_CONSTRAINTS, ArrayUtils.toString(currentValues.toArray()));
	}
	...
	private Set<String> getConstraintNameList(){
		Set<String> result = new HashSet<String>();
		String propertyValue = getProperty(ProcessContextProperty.ALL_CONSTRAINTS);
		if(propertyValue == null)
			return result;
	...
	private AbstractConstraint<?> getConstraint(String constraintName) throws PropertyException{
		...
		if(constraintString == null)
			throw new PropertyException(ProcessContextProperty.CONSTRAINT, constraintName, "Unparseable constraint");
