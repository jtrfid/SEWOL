...
public class LogEntry implements Comparable<LogEntry>, Cloneable {
    ...
    /**
     * The timestamp of the log entry (the time the related activity was
     * conducted).<br>
     */
     ...
     * Activities can be related to a set of different events such as
     * <i>start</i>,<i>complete</i>, etc.
     */
    protected EventType eventType = null;
    /**
     * A list of additional meta information for the log entry.
     */
    protected Set<DataAttribute> metaData = new HashSet<>();
    /**
     * Stores the locked fields of the log entry together with the locking
     * reason.<br>
     * Locked fields cannot be altered any more.
     */
    private HashMap<EntryField, String> locking = new HashMap<>();
    /**
     * Random generator.
     */
    protected Random rand = new Random();
    /**
     * Field that allows to group log traces together by assigning them the same
     * group.<br>
     * Can be useful when different event types for an activity are used (start,
     * suspend, complete). Group assignment in this case allows to reflect
     * togetherness.
     */
     ...
     * @throws LockingException if the field TIME is locked and the given
     * timestamp differs from the current value of {@link #timestamp}.
     */
     ...
     * @throws LockingException if the field ACTIVITY is locked and the given
     * activity differs from the current value of {@link #activity}.
     ...
     * @throws LockingException if the originator field is locked <br>and the
     * given value differs from the actual value of {@link #originator}.
     ...
     * @throws ParameterException if the given value is <code>null</code>.
     * @throws LockingException if the role field is locked <br>and the given
     * value differs from the actual value of {@link #role}.
     ...
     * @throws LockingException if the field EVENTTYPE is locked and the given
     * event type differs from {@link #eventType}.
	...
	//------- Group -----------------------------------------------------------------------------
    /**
     * Returns the group assigned to this entry or <code>null</code>. Setting
     * group IDs for entries allows to reflect relations and togetherness
     * amongst entries within a trace.
     *
     * @return The group ID.
     * @see #setGroup(String)
     ...
     * Setting group IDs for entries allows to reflect relations and
     * togetherness amongst entries within a trace.
     *
     ...
     * Locked fields cannot be altered anymore. If the originator field is
     * locked, then the originator candidates field is locked as well, since
     * changing the candidates also changes the originator (consistency).
     *
     * @param entryField
     * @param reason
     */
    ...
    public String toString() {
        String ts = (this.timestamp == null) ? "-" : sdf.format(this.timestamp);
        return String.format(toStringFormat, ts, getActivity(), getOriginator());
    }
